...
namespace PriorityQueue
{
    public sealed class SimplePriorityQueue<T> : IPriorityQueue<T>
    {
        ...
        private readonly FastPriorityQueue<SimpleNode> MyQueue;

        public SimplePriorityQueue()
        {
            MyQueue = new FastPriorityQueue<SimpleNode>(INITIAL_QUEUE_SIZE);
        }
        ...
        public T First
        {
            get
            {
                lock(MyQueue)
                {
                    ...
                    SimpleNode first = MyQueue.First;
        ...
        public T Dequeue()
        {
            lock(MyQueue)
            {
                ...
                SimpleNode node =MyQueue.Dequeue();
                return node.Data;
            }
        }
        ...
        public void Enqueue(T item, double priority)
        {
            lock(MyQueue)
            {
                ...
                {
                    MyQueue.Resize(MyQueue.MaxSize*2 + 1);
                }
                MyQueue.Enqueue(node, priority);
            }
        }
        ...
        public void Remove(T item)
        {
            lock(MyQueue)
            {
                try
                {
                    MyQueue.Remove(GetExistingNode(item));
                }
        ...
        public void UpdatePriority(T item, double priority)
        {
            lock (MyQueue)
            {
                try
                {
                    ...
                    MyQueue.UpdatePriority(updateMe, priority);
                }
                catch(InvalidOperationException ex)
                {
